...
\NormalTok{  el}\OperatorTok{:}\NormalTok{ (row) }\KeywordTok{=>} \VariableTok{row}\OperatorTok{.}\FunctionTok{querySelector}\NormalTok{(}\VerbatimStringTok{\textasciigrave{}.\_1ebt2xej\textasciigrave{}}\NormalTok{)}\OperatorTok{,}
\OperatorTok{\}}
\end{Highlighting}
\end{Shaded}
...
most DOM manipulation is not performed directly by the site
...
which are implemented as Javascript functions that can perform arbitrary
behaviors like simulating clicks on buttons or mutating the DOM.
...
recognizing these changes by observing the DOM. So far, we have been
able to use event listeners and the MutationObserver API to successfully
observe changes, but it may prove challenging to correctly respond to
updates on some sites purely by observing the DOM.
...
Users get high leverage from mastering a text editor and other text
manipulation tools, since they can be applied to a huge variety of
tasks. A user's preferred text editor can even serve as an interactive
input mechanism in shell programs, e.g.~for editing git commit messages.
...
notes that textual literacy depends on the fact that writing can be
...
from essays to emails, the medium would lose its potency. Having generic
tools is especially important for software customization, because a
common barrier to customizing software is not having enough time
\autocite{mackay1991}. It's more likely that people will customize
software regularly if they can reuse the same tools across many
applications.
...
someone is able to write a program that helps them achieve a useful
task.
...
emerge in practice. Section~\ref{sec:demos} also discusses some
limitations we expect to face.
...
structured data in the page. (It would certainly be far simpler than
implementing a third-party site adapter.) There is also precedent for
first parties implementing an official client extension API: for several
years, Google maintained an official extension API in Gmail for
Greasemonkey scripts to use.
...
single adapter for a site and keep it updated in response to changes to
the site, rather than each programmer creating custom scraping logic for
their own individual extensions.
...
way for end users to express imperative workflows with sequences of
actions in Wildcard. Chickenfoot and Coscripter have shown these types
of workflows to be useful in practice, but it's not clear how such
workflows could fit into Wildcard. Spreadsheets have a fundamentally
different computation model, and site adapters created by programmers
cannot easily account for every possible action a user might want to
perform in a page.
