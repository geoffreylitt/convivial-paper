...
* *Reliability*: It can be difficult to know whether a command will consistently work over time. In addition to changes to the website, changes to the system's heuristics can also cause unexpected changes in behavior.
...
We hope that spreadsheet-driven customization can help the Web reach its full potential as a dynamic medium that users can mold to their own needs.
